...
use tokio::sync::mpsc;
use zfx_socket_actix::active::IncomingMsg;
use zfx_socket_actix::active::Listener;
use zfx_socket_actix::active::Socket;
use zfx_socket_actix::Error;
        ...
        tokio::spawn(async move {
            ...
            loop {
                match send_rx.recv().await {
                    Some(TestMessage::Send) => {
                        raw_socket.send(b"server-origin message").await.unwrap()
                    }
...
async fn simple_connection_lifetime() {
    ...
    assert_eq!(client.send(b"sendpanictest").await.unwrap(), ());
